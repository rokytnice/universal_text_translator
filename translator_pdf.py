...
def create_pdf_with_response(page_content, response):
    ...
    Schreibt keine leeren oder abgeschnittenen Übersetzungen ins PDF.
    """
    if not response.strip():
        print("[WARNING] Skipping empty or truncated translation.")
        return None
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
        ...
        translated_paragraph = Paragraph("" + response.replace("\n", "<br />"), normal_style)
...
def process_pdf(input_path, output_path, model, start_page, end_page):
    ...
    for page_number, page in enumerate(reader.pages):
        ...
        response_pdf_path = create_pdf_with_response(page_content, ai_response)

        if response_pdf_path:
            # Neue Seite zur Ausgabe hinzufügen
            try:
                with open(response_pdf_path, 'rb') as pdf_file:
                    response_reader = PdfReader(pdf_file)
                    writer.add_page(response_reader.pages[0])
            except Exception as e:
                print(f"[ERROR] Could not read the generated PDF: {e}")
